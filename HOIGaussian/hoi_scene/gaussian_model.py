...
from utils.loss_utils import chamfer_distance
...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, smpl_type : str, motion_offset_flag : bool, actor_gender: str):
        ...
        ## wbr
        self.x_angle= torch.empty(0)
        self.y_angle= torch.empty(0)
        self.z_angle= torch.empty(0)
        self.scale_obj=torch.empty(0)
        self.translation_x=torch.empty(0)
        self.translation_y=torch.empty(0)
        self.translation_z=torch.empty(0)

        # wbr
        self.contact= torch.empty(0)
        self.cf_distance= torch.empty(0)
        self.contact_alpha= torch.empty(0)
        self.contact_beta= torch.empty(0)
        self.h_xyz= torch.empty(0)
    ...
    def get_transform_obj(self):
        x_angle = self.x_angle[0]
        y_angle = self.y_angle[0]
        z_angle = self.z_angle[0]

        RX = torch.stack([
            torch.tensor([1, 0, 0], device=x_angle.device),
            torch.stack([torch.tensor(0, device=x_angle.device), torch.cos(x_angle), -torch.sin(x_angle)]),
            torch.stack([torch.tensor(0, device=x_angle.device), torch.sin(x_angle), torch.cos(x_angle)])
        ])

        RY = torch.stack([
            torch.stack([torch.cos(y_angle), torch.tensor(0, device=y_angle.device), torch.sin(y_angle)]),
            torch.tensor([0, 1, 0], device=y_angle.device),
            torch.stack([-torch.sin(y_angle), torch.tensor(0, device=y_angle.device), torch.cos(y_angle)])
        ])

        RZ = torch.stack([
            torch.stack([torch.cos(z_angle), -torch.sin(z_angle), torch.tensor(0, device=z_angle.device)]),
            torch.stack([torch.sin(z_angle), torch.cos(z_angle), torch.tensor(0, device=z_angle.device)]),
            torch.tensor([0, 0, 1], device=z_angle.device)
        ])
    ...
    @property
    def get_transl_obj(self):
        trans_x=self.translation_x
        trans_y=self.translation_y
        trans_z=self.translation_z
        translation=torch.stack([trans_x,trans_y,trans_z]).view(1,3)
        return translation
    ...
    def create_from_pcd(self, pcd : BasicPointCloud,obj_pcd : BasicPointCloud, spatial_lr_scale : float,save_obj:np.ndarray):


        ## wbr add obj_pcd
        self.spatial_lr_scale = spatial_lr_scale
        human_pcd=torch.tensor(np.asarray(pcd.points)).float().cuda()
        o_pcd=torch.tensor(np.asarray(obj_pcd.points)).float().cuda()
        fused_point_cloud = torch.tensor(np.concatenate([np.asarray(pcd.points),np.asarray(obj_pcd.points)],axis=0)).float().cuda()
        fused_color = RGB2SH(torch.tensor(np.concatenate([np.asarray(pcd.colors),np.asarray(obj_pcd.colors)],axis=0)).float().cuda())
        ...
        dist2h = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000001)
        dist2o= torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(obj_pcd.points)).float().cuda()), 0.0000001)
        scales_h = torch.log(torch.sqrt(dist2h*0.1))[...,None].repeat(1, 3)
        scales_o = torch.log(torch.sqrt(dist2o))[...,None].repeat(1, 3)
        scales= torch.cat((scales_h,scales_o),dim=0)
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        rots[:, 0] = 1

        contact = torch.tensor(pcd.contacts).int().cuda()
        # print(contact)
        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        # print(torch.sum(contact))


        print('contact',torch.sum(contact))
        opacities[contact.bool()] = inverse_sigmoid(torch.tensor(1e-5).cuda())
        opacity_h = opacities[:10475]

        self.save_obj=save_obj
        # opacity_h = h_distance



        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))
        self._h_xyz= nn.Parameter(human_pcd.requires_grad_(True))
        self._o_xyz= nn.Parameter(o_pcd.requires_grad_(True))
        ...
        ## wbr obj transform
        self.x_angle= nn.Parameter(torch.tensor([0.0]).cuda(), requires_grad=True)
        self.y_angle= nn.Parameter(torch.tensor([0.0]).cuda(), requires_grad=True)
        self.z_angle= nn.Parameter(torch.tensor([0.0]).cuda(), requires_grad=True)
        self.scale_obj= nn.Parameter(torch.tensor([1.0]).cuda(), requires_grad=True)
        self.translation_x= nn.Parameter(torch.tensor([0.0]).cuda(), requires_grad=True)
        self.translation_y= nn.Parameter(torch.tensor([0.0]).cuda(), requires_grad=True)
        self.translation_z= nn.Parameter(torch.tensor([0.0]).cuda(), requires_grad=True)

        ## wbr chamfer distance

        h_distance,o_distance=chamfer_distance(self._h_xyz.unsqueeze(0),self._o_xyz.unsqueeze(0))
        self.cf_distance=torch.concatenate([h_distance,o_distance],dim=1).cuda()



        # wbr contact assets
        # self.contact_alpha= nn.Parameter(torch.tensor([0.5]).cuda(), requires_grad=True)
        # self.contact_beta= nn.Parameter(torch.tensor([0.5]).cuda(), requires_grad=True)
        self.contact_alpha=torch.tensor(0.2).cuda()
        self.contact_beta=torch.tensor(0.8).cuda()
        # self.contact= self.contact_alpha*self._opacity+self.contact_beta*self.cf_distance
    ...
    def training_setup(self, training_args):
        ...
        if not self.motion_offset_flag:
            l = [
                ...
                {'params': [self._scaling], 'lr': training_args.scaling_lr/1000.0, "name": "scaling"},
                {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"}
            ]
        else:
            l = [
                {'params': [self.x_angle], 'lr': training_args.angle_lr_init * self.spatial_lr_scale, "name": "x_angle"},
                {'params': [self.y_angle], 'lr': training_args.angle_lr_init * self.spatial_lr_scale, "name": "y_angle"},
                {'params': [self.z_angle], 'lr': training_args.angle_lr_init * self.spatial_lr_scale, "name": "z_angle"},
                {'params': [self.translation_x], 'lr': training_args.obj_lr_init * self.spatial_lr_scale, "name": "translation_x"},
                {'params': [self.translation_y], 'lr': training_args.obj_lr_init * self.spatial_lr_scale, "name": "translation_y"},
                {'params': [self.translation_z], 'lr': training_args.obj_lr_init * self.spatial_lr_scale, "name": "translation_z"},
                {'params': [self.scale_obj], 'lr': training_args.obj_scale_init * self.spatial_lr_scale, "name": "scale"},
                {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale/10000.0, "name": "xyz"},
                {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
                {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
                {'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"},
                {'params': [self._scaling], 'lr': training_args.scaling_lr/1000.0, "name": "scaling"},
    ...
    def obj_transform_gs(self, query_pts, R, transl,scale):
        query_center= torch.mean(query_pts, dim=0, keepdim=True)
        # print(query_center)
        query_pts = query_pts - query_center
        query_pts = query_pts * scale
        query_pts=torch.matmul(query_pts, R)
        query_pts = query_pts + query_center+ transl
        return query_pts
